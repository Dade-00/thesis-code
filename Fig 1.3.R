...
ggplot(Salaries, aes(x=yrs.service, y = salary)) +
  geom_boxplot(aes(group = experience_group),
               alpha = 0.7, fill = "gray80",outlier.shape = NA) +  # Boxplot uniformi
  geom_line(data = predictions, aes(x = exp(log_exp), y = salary_pred, color = factor(tau)), size = 1) +  # Curve allineate
  labs(
